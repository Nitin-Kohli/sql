CREATE PROCEDURE UPDATE_REPORT3(
@USERINPUT INT) AS
BEGIN
	...
	SELECT  @PIVOT_HEADER = (
	SELECT TOP (SELECT @USERINPUT) 
	',' +  QUOTENAME(ROW_NUMBER() OVER (ORDER BY (SELECT NULL))) FROM MAIN FOR XML PATH ('')  
	);
	SET @PIVOT_HEADER = STUFF(@PIVOT_HEADER,1,1,'');
